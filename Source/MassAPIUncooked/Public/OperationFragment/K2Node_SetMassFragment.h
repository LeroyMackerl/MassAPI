...
#include "K2Node_SetMassFragment.generated.h"
...
UCLASS()
class MASSAPIUNCOOKED_API UK2Node_SetMassFragment : public UK2Node
{
	GENERATED_BODY()
...
public:
	...
	/** 是否设置结构体成员（如果为True，则显示成员选择而非Fragment引脚） */
	UPROPERTY(EditAnywhere, Category = "MemberSelection")
	bool bSetMember = false;

	/** 结构体成员引用（支持递归嵌套和多选，当bSetMember=True时使用） */
	UPROPERTY(EditAnywhere, Category = "MemberSelection", meta = (EditCondition = "bSetMember"))
