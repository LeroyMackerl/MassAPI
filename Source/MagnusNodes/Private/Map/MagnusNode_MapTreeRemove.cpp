#include "Map/MagnusNode_MapTreeRemove.h"
...
FText UMagnusNode_MapTreeRemove::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	switch (ContainerType)
	{
		case EMapTreeContainerType::NoContainer:
			return FText::FromString(TEXT("MapTreeRemove"));
		case EMapTreeContainerType::Array:
			return FText::FromString(TEXT("RemoveItem-Array"));
		case EMapTreeContainerType::Map:
			return FText::FromString(TEXT("RemoveItem-Map"));
		case EMapTreeContainerType::Set:
			return FText::FromString(TEXT("RemoveItem-Set"));
		default:
			return FText::FromString(TEXT("MapTreeRemove"));
	}
}

FText UMagnusNode_MapTreeRemove::GetCompactNodeTitle() const
{
	if (ContainerType == EMapTreeContainerType::NoContainer)
	{
		return FText::FromString(TEXT("MAPTREE"));
	}
	return FText::FromString(TEXT("REMOVEITEM"));
}

FText UMagnusNode_MapTreeRemove::GetTooltipText() const
{
	switch (ContainerType)
	{
		case EMapTreeContainerType::NoContainer:
		...
		case EMapTreeContainerType::Array:
			return FText::FromString(TEXT("Remove a item from array in map struct value"));
		case EMapTreeContainerType::Map:
			return FText::FromString(TEXT("Remove a item from map in map struct value"));
		case EMapTreeContainerType::Set:
			return FText::FromString(TEXT("Remove a item from set in map struct value"));
		default:
			return FText::FromString(TEXT("Map tree operation"));
	}
}

FText UMagnusNode_MapTreeRemove::GetMenuCategory() const
{
	return FText::FromString(TEXT("Utilities|Magnus"));
}

FSlateIcon UMagnusNode_MapTreeRemove::GetIconAndTint(FLinearColor& OutColor) const
{
	static FSlateIcon Icon("EditorStyle", "GraphEditor.MakeMap_16x");
	return Icon;
}

TSharedPtr<SWidget> UMagnusNode_MapTreeRemove::CreateNodeImage() const
{
...
void UMagnusNode_MapTreeRemove::AllocateDefaultPins()
{
	// 创建执行引脚（所有模式都有）
	...
	if (ContainerType != EMapTreeContainerType::NoContainer)
	{
		...
		// 创建 Value 输入引脚（Array 和 Set 需要）
		if (ContainerType == EMapTreeContainerType::Array || ContainerType == EMapTreeContainerType::Set)
		{
...
void UMagnusNode_MapTreeRemove::PinDefaultValueChanged(UEdGraphPin* Pin)
{
	Super::PinDefaultValueChanged(Pin);
	PropagatePinType();
}

void UMagnusNode_MapTreeRemove::ReconstructNode()
{
	Super::ReconstructNode();
	PropagatePinType();
}

void UMagnusNode_MapTreeRemove::PostReconstructNode()
{
	Super::PostReconstructNode();
	PropagatePinType();
}

void UMagnusNode_MapTreeRemove::PinConnectionListChanged(UEdGraphPin* Pin)
{
...
void UMagnusNode_MapTreeRemove::NotifyPinConnectionListChanged(UEdGraphPin* Pin)
{
...
bool UMagnusNode_MapTreeRemove::IsConnectionDisallowed(const UEdGraphPin* MyPin, const UEdGraphPin* OtherPin, FString& OutReason) const
{
	// 先检查参数有效性
	if (!MyPin || !OtherPin)
	{
...
FEdGraphPinType UMagnusNode_MapTreeRemove::GetKeyPinType() const
{
...
FEdGraphPinType UMagnusNode_MapTreeRemove::GetSubKeyPinType() const
{
	FEdGraphPinType PinType;
	PinType.PinCategory = UEdGraphSchema_K2::PC_Wildcard;
...
FEdGraphPinType UMagnusNode_MapTreeRemove::GetItemPinType() const
{
	FEdGraphPinType PinType;
	PinType.PinCategory = UEdGraphSchema_K2::PC_Wildcard;
...
void UMagnusNode_MapTreeRemove::PropagatePinType()
{
	UEdGraphPin* MapPin = FindPin(InputMapPinName());
...
#if WITH_EDITOR
void UMagnusNode_MapTreeRemove::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.Property)
	{
		...
		if (PropertyName == GET_MEMBER_NAME_CHECKED(UMagnusNode_MapTreeRemove, ContainerType))
		{
		...
		bool bIsMemberReferenceChanged =
			(MemberPropertyName == GET_MEMBER_NAME_CHECKED(UMagnusNode_MapTreeRemove, MemberReference)) ||
...
void UMagnusNode_MapTreeRemove::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
...
HNCH_StartExpandNode(UMagnusNode_MapTreeRemove)
	...
	virtual void Compile() override
	{
		...
		if (!OwnerNode->MemberReference.IsValid() || OwnerNode->ContainerType == EMapTreeContainerType::NoContainer)
		{
			CompilerContext.MessageLog.Error(
				*NSLOCTEXT("K2Node", "MapTreeRemove_InvalidMember", "MapTreeRemove: 请先选择一个有效的结构体成员！").ToString(),
				OwnerNode
			);
			// 仍然连接执行流，避免蓝图编译失败
		...
		switch (OwnerNode->ContainerType)
		{
			...
			case EMapTreeContainerType::Array:
			{
				...
				// 1. 创建 MapTree_RemoveArrayItem 函数节点
				UK2Node_CallFunction* CallNode = HNCH_SpawnFunctionNode(UMagnusFuncLib_MapTree, MapTree_RemoveArrayItem);
			...
			case EMapTreeContainerType::Set:
			{
				...
				// 1. 创建 MapTree_RemoveSetItem 函数节点
				UK2Node_CallFunction* CallNode = HNCH_SpawnFunctionNode(UMagnusFuncLib_MapTree, MapTree_RemoveSetItem);
			...
			case EMapTreeContainerType::Map:
			{
				...
				// 1. 创建 MapTree_RemoveMapItem 函数节点
				UK2Node_CallFunction* CallNode = HNCH_SpawnFunctionNode(UMagnusFuncLib_MapTree, MapTree_RemoveMapItem);

				// 2. 连接执行引脚
				Link(ProxyExecPin(), ExecPin(CallNode));
				Link(ThenPin(CallNode), ProxyThenPin());

				// 3. 连接数据引脚：TargetMap, Key, MemberName, SubKey
				Link(ProxyPin(OwnerNode->InputMapPinName()), FunctionInputPin(CallNode, "TargetMap"));
				Link(ProxyPin(OwnerNode->InputKeyPinName()), FunctionInputPin(CallNode, "Key"));
				...
				// Map 特定：连接 SubKey 参数（注意：Remove 不需要 Value）
...
HNCH_EndExpandNode(UMagnusNode_MapTreeRemove)
