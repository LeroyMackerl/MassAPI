#include "Map/MagnusNode_MapTreeContains.h"
...
FText UMagnusNode_MapTreeContains::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	switch (ContainerType)
	{
		case EMapTreeContainerType::NoContainer:
			return FText::FromString(TEXT("MapTreeContains"));
		case EMapTreeContainerType::Array:
			return FText::FromString(TEXT("ContainsItem-Array"));
		case EMapTreeContainerType::Map:
			return FText::FromString(TEXT("ContainsItem-Map"));
		case EMapTreeContainerType::Set:
			return FText::FromString(TEXT("ContainsItem-Set"));
		default:
			return FText::FromString(TEXT("MapTreeContains"));
	}
}

FText UMagnusNode_MapTreeContains::GetCompactNodeTitle() const
{
	if (ContainerType == EMapTreeContainerType::NoContainer)
	{
		return FText::FromString(TEXT("MAPTREE"));
	}
	return FText::FromString(TEXT("CONTAINS"));
}

FText UMagnusNode_MapTreeContains::GetTooltipText() const
{
	switch (ContainerType)
	{
		case EMapTreeContainerType::NoContainer:
		...
		case EMapTreeContainerType::Array:
			return FText::FromString(TEXT("Check if array in map struct value contains a item"));
		case EMapTreeContainerType::Map:
			return FText::FromString(TEXT("Check if map in map struct value contains a item"));
		case EMapTreeContainerType::Set:
			return FText::FromString(TEXT("Check if set in map struct value contains a item"));
		default:
			return FText::FromString(TEXT("Map tree operation"));
	}
}

FText UMagnusNode_MapTreeContains::GetMenuCategory() const
{
	return FText::FromString(TEXT("Utilities|Magnus"));
}

FSlateIcon UMagnusNode_MapTreeContains::GetIconAndTint(FLinearColor& OutColor) const
{
	static FSlateIcon Icon("EditorStyle", "GraphEditor.MakeMap_16x");
	return Icon;
}

TSharedPtr<SWidget> UMagnusNode_MapTreeContains::CreateNodeImage() const
{
...
void UMagnusNode_MapTreeContains::AllocateDefaultPins()
{
	// 纯函数节点不需要执行引脚
...
void UMagnusNode_MapTreeContains::PinDefaultValueChanged(UEdGraphPin* Pin)
{
	Super::PinDefaultValueChanged(Pin);
	PropagatePinType();
}

void UMagnusNode_MapTreeContains::ReconstructNode()
{
	Super::ReconstructNode();
	PropagatePinType();
}

void UMagnusNode_MapTreeContains::PostReconstructNode()
{
	Super::PostReconstructNode();
	PropagatePinType();
}

void UMagnusNode_MapTreeContains::PinConnectionListChanged(UEdGraphPin* Pin)
{
...
void UMagnusNode_MapTreeContains::NotifyPinConnectionListChanged(UEdGraphPin* Pin)
{
...
bool UMagnusNode_MapTreeContains::IsConnectionDisallowed(const UEdGraphPin* MyPin, const UEdGraphPin* OtherPin, FString& OutReason) const
{
	// 先检查参数有效性
	if (!MyPin || !OtherPin)
	{
...
FEdGraphPinType UMagnusNode_MapTreeContains::GetKeyPinType() const
{
...
FEdGraphPinType UMagnusNode_MapTreeContains::GetSubKeyPinType() const
{
	FEdGraphPinType PinType;
	PinType.PinCategory = UEdGraphSchema_K2::PC_Wildcard;
...
FEdGraphPinType UMagnusNode_MapTreeContains::GetItemPinType() const
{
	FEdGraphPinType PinType;
	PinType.PinCategory = UEdGraphSchema_K2::PC_Wildcard;
...
void UMagnusNode_MapTreeContains::PropagatePinType()
{
	UEdGraphPin* MapPin = FindPin(InputMapPinName());
...
#if WITH_EDITOR
void UMagnusNode_MapTreeContains::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.Property)
	{
		...
		if (PropertyName == GET_MEMBER_NAME_CHECKED(UMagnusNode_MapTreeContains, ContainerType))
		{
		...
		bool bIsMemberReferenceChanged =
			(MemberPropertyName == GET_MEMBER_NAME_CHECKED(UMagnusNode_MapTreeContains, MemberReference)) ||
...
void UMagnusNode_MapTreeContains::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
...
HNCH_StartExpandNode(UMagnusNode_MapTreeContains)
	...
	virtual void Compile() override
	{
		...
		if (!OwnerNode->MemberReference.IsValid() || OwnerNode->ContainerType == EMapTreeContainerType::NoContainer)
		{
			CompilerContext.MessageLog.Error(
				*NSLOCTEXT("K2Node", "MapTreeContains_InvalidMember", "MapTreeContains: 请先选择一个有效的结构体成员！").ToString(),
				OwnerNode
			);
			// 纯函数节点没有执行流，直接返回false
			if (UEdGraphPin* ResultPin = ProxyPin(OwnerNode->OutputResultPinName()))
			{
		...
		switch (OwnerNode->ContainerType)
		{
			...
			case EMapTreeContainerType::Array:
			{
				...
				// 1. 创建 MapTree_ContainsArrayItem 函数节点
				UK2Node_CallFunction* CallNode = HNCH_SpawnFunctionNode(UMagnusFuncLib_MapTree, MapTree_ContainsArrayItem);

				// 2. 连接数据引脚：TargetMap, Key, MemberName, Item
				...
				// 3. 连接返回值
				Link(FunctionReturnPin(CallNode), ProxyPin(OwnerNode->OutputResultPinName()));
			...
			case EMapTreeContainerType::Set:
			{
				...
				// 1. 创建 MapTree_ContainsSetItem 函数节点
				UK2Node_CallFunction* CallNode = HNCH_SpawnFunctionNode(UMagnusFuncLib_MapTree, MapTree_ContainsSetItem);
			...
			case EMapTreeContainerType::Map:
			{
				...
				// 1. 创建 MapTree_ContainsMapItem 函数节点
				UK2Node_CallFunction* CallNode = HNCH_SpawnFunctionNode(UMagnusFuncLib_MapTree, MapTree_ContainsMapItem);

				// 2. 连接数据引脚：TargetMap, Key, MemberName, SubKey, Value
				Link(ProxyPin(OwnerNode->InputMapPinName()), FunctionInputPin(CallNode, "TargetMap"));
				Link(ProxyPin(OwnerNode->InputKeyPinName()), FunctionInputPin(CallNode, "Key"));
...
HNCH_EndExpandNode(UMagnusNode_MapTreeContains)
