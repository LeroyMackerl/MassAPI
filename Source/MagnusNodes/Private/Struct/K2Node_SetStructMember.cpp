...
void UK2Node_SetStructMember::AllocateDefaultPins()
{
	// 创建执行引脚
	CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Exec, UK2Node_SetStructMember::ExecPinName());
	CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Exec, UK2Node_SetStructMember::ThenPinName());

	// 创建输入引脚：Struct (根据 MemberReference 确定类型)
	UEdGraphPin* StructInputPin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Wildcard, UK2Node_SetStructMember::StructPinName());
	StructInputPin->PinType.bIsReference = true;

	// 创建输入引脚：FieldValue (根据 MemberReference 确定类型)
	CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Wildcard, UK2Node_SetStructMember::MemberPinName());
...
void UK2Node_SetStructMember::ReallocatePinsDuringReconstruction(TArray<UEdGraphPin*>& OldPins)
{
	Super::ReallocatePinsDuringReconstruction(OldPins);
}

void UK2Node_SetStructMember::PostReconstructNode()
{
...
void UK2Node_SetStructMember::PinDefaultValueChanged(UEdGraphPin* Pin)
{
	Super::PinDefaultValueChanged(Pin);
}

void UK2Node_SetStructMember::PinTypeChanged(UEdGraphPin* Pin)
{
...
bool UK2Node_SetStructMember::IsConnectionDisallowed(const UEdGraphPin* MyPin, const UEdGraphPin* OtherPin, FString& OutReason) const
{
	// 如果是输入引脚且为 Wildcard，不允许连接
	if (MyPin && MyPin->PinName == UK2Node_SetStructMember::MemberPinName() && MyPin->Direction == EGPD_Input)
	{
	...
	// 限制输入引脚只能连接正确的结构体类型
	if (MyPin && MyPin->PinName == UK2Node_SetStructMember::StructPinName())
	{
...
void UK2Node_SetStructMember::NotifyPinConnectionListChanged(UEdGraphPin* Pin)
{
	Super::NotifyPinConnectionListChanged(Pin);

	// 当 Struct 引脚连接改变时，同步类型并更新 MemberReference
	if (Pin && Pin->PinName == UK2Node_SetStructMember::StructPinName())
	{
...
void UK2Node_SetStructMember::ValidateNodeDuringCompilation(class FCompilerResultsLog& MessageLog) const
{
	...
	{
		MessageLog.Error(*FString::Printf(TEXT("Member '%s' is read-only and cannot be set. @@"),
			*StructMemberReference.MemberPath), this);
	}
}
...
UScriptStruct* UK2Node_SetStructMember::GetStructType() const
{
	// 只从 MemberReference 获取结构体类型
	return StructMemberReference.StructType.Get();
}

FProperty* UK2Node_SetStructMember::GetPropertyFromMemberReference() const
{
	if (!StructMemberReference.IsValid())
	{
		return nullptr;
...
void UK2Node_SetStructMember::SetMemberPath(const FString& InMemberPath)
{
	StructMemberReference.MemberPath = InMemberPath;
	OnMemberReferenceChanged();
}

void UK2Node_SetStructMember::OnMemberReferenceChanged()
{
	// 更新输入引脚类型
	if (UEdGraphPin* StructInputPin = FindPin(UK2Node_SetStructMember::StructPinName()))
	{
		UScriptStruct* StructType = GetStructType();
	...
	// 更新 FieldValue 输入引脚类型
	UEdGraphPin* FieldInputPin = FindPin(UK2Node_SetStructMember::MemberPinName());
	if (!FieldInputPin)
	{
		return;
	}
	...
	// 如果 MemberReference 无效，设置输入为 Wildcard
	if (!StructMemberReference.IsValid())
	{
		if (FieldInputPin->PinType.PinCategory != UEdGraphSchema_K2::PC_Wildcard)
		{
			FieldInputPin->BreakAllPinLinks();
			FieldInputPin->PinType.PinCategory = UEdGraphSchema_K2::PC_Wildcard;
			FieldInputPin->PinType.PinSubCategoryObject = nullptr;
			FieldInputPin->PinFriendlyName = FText::GetEmpty();
			bPinTypeChanged = true;
		}
	}
	else
	{
		// 有 MemberReference，更新输入引脚类型
		...
		if (Property && StructMemberHelper::CanCreatePinForProperty(Property))
		{
			...
			{
				// 检查类型是否改变
				if (FieldInputPin->PinType != NewPinType)
				{
					// 类型不匹配，断开连接
					if (FieldInputPin->LinkedTo.Num() > 0)
					{
						FieldInputPin->BreakAllPinLinks();
					}

					FieldInputPin->PinType = NewPinType;
					FieldInputPin->PinType.bIsReference = true;  // 使用引用传递，避免复制
					bPinTypeChanged = true;
				}
				...
				if (!FieldInputPin->PinFriendlyName.EqualTo(NewFriendlyName))
				{
					FieldInputPin->PinFriendlyName = NewFriendlyName;
...
void UK2Node_SetStructMember::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyName == GET_MEMBER_NAME_CHECKED(UK2Node_SetStructMember, StructMemberReference) ||
		MemberPropertyName == GET_MEMBER_NAME_CHECKED(UK2Node_SetStructMember, StructMemberReference))
	{
		// 立即更新引脚
		OnMemberReferenceChanged();
	}
...
class FKCHandler_SetStructMemberField : public FNodeHandlingFunctor
{
public:
	FKCHandler_SetStructMemberField(FKismetCompilerContext& InCompilerContext)
		: FNodeHandlingFunctor(InCompilerContext)
	{
	}

	FBPTerminal* RegisterInputTerm(FKismetFunctionContext& Context, UK2Node_SetStructMember* Node)
	{
		check(Node);
		...
		// 找到输入引脚
		UEdGraphPin* InputPin = Node->FindPin(UK2Node_SetStructMember::StructPinName());
		if (!InputPin)
		{
	...
	FBPTerminal* RegisterValueTerm(FKismetFunctionContext& Context, UK2Node_SetStructMember* Node, UScriptStruct* StructType)
	{
		...
		{
			CompilerContext.MessageLog.Error(*FString::Printf(TEXT("Member '%s' is read-only and cannot be set. @@"),
				*Node->StructMemberReference.MemberPath), Node);
			return nullptr;
		}

		if (BoundProperty->HasAnyPropertyFlags(CPF_Deprecated))
		{
		...
		// 获取输入值引脚
		UEdGraphPin* ValueInputPin = Node->FindPin(UK2Node_SetStructMember::MemberPinName());
		if (!ValueInputPin)
		{
			CompilerContext.MessageLog.Error(TEXT("Missing value input pin @@"), Node);
			return nullptr;
		}

		// 注册输入值 terminal
		UEdGraphPin* ValueNet = FEdGraphUtilities::GetNetFromPin(ValueInputPin);
		check(ValueNet);

		FBPTerminal** FoundValueTerm = Context.NetMap.Find(ValueNet);
		FBPTerminal* ValueTerm = FoundValueTerm ? *FoundValueTerm : nullptr;

		if (!ValueTerm)
		{
			ValueTerm = Context.CreateLocalTerminalFromPinAutoChooseScope(ValueNet, Context.NetNameMap->MakeValidName(ValueNet));
			Context.NetMap.Add(ValueNet, ValueTerm);
		}
	...
	{
		UK2Node_SetStructMember* Node = Cast<UK2Node_SetStructMember>(InNode);
		check(Node);
		...
		// 注册输入引脚
		RegisterInputTerm(Context, Node);

		// 注册值引脚
		RegisterValueTerm(Context, Node, StructType);
	}
	...
	virtual void Compile(FKismetFunctionContext& Context, UEdGraphNode* InNode) override
	{
		...
		// 获取结构体 terminal
		UEdGraphPin* StructInputPin = Node->FindPin(UK2Node_SetStructMember::StructPinName());
		UEdGraphPin* StructNet = FEdGraphUtilities::GetNetFromPin(StructInputPin);
		FBPTerminal** FoundStructTerm = Context.NetMap.Find(StructNet);
		if (!FoundStructTerm || !*FoundStructTerm)
		{
			return;
		}
		FBPTerminal* StructContextTerm = *FoundStructTerm;

		// 获取值 terminal
		UEdGraphPin* ValueInputPin = Node->FindPin(UK2Node_SetStructMember::MemberPinName());
		UEdGraphPin* ValueNet = FEdGraphUtilities::GetNetFromPin(ValueInputPin);
		FBPTerminal** FoundValueTerm = Context.NetMap.Find(ValueNet);
		if (!FoundValueTerm || !*FoundValueTerm)
		{
			return;
		}
...
FNodeHandlingFunctor* UK2Node_SetStructMember::CreateNodeHandler(class FKismetCompilerContext& CompilerContext) const
{
	return new FKCHandler_SetStructMemberField(CompilerContext);
}
